...
from detector_tracker_radical import DetectorTracker
import constants
...
def run_tests(sess, detection_graph):
    ...
    with open(constants.DATA_FILE, "w") as f:
        ...
        writer.writerow(constants.DATA_FORMAT[:1]+constants.DATA_FORMAT[2:])

        for DETECTION_CYCLE in ['5','10','3']:
            for UNTRACKED_THRESH in ['3','5','10']:
                for PIXEL_LIMIT in ['150','100','200']:
                    for in_file in constants.VIDEOS:
                        print('Running on %s for parameters --detection-cycle %s --score-threshold %s --association-buffer %s --pixel-limit %s --im-zoom %s' % (in_file, DETECTION_CYCLE, MIN_SCORE_THRESH, UNTRACKED_THRESH, PIXEL_LIMIT, IM_ZOOM))
                        file_path = os.path.join(constants.DATA_DIR, in_file)

                        detector = DetectorTracker(sess, detection_graph, tuple([file_path, DETECTION_CYCLE, MIN_SCORE_THRESH, UNTRACKED_THRESH, PIXEL_LIMIT, IM_ZOOM]))
                        detector.analyze()
                        count = detector.get_count()

                        newrow = [DETECTION_CYCLE, UNTRACKED_THRESH, PIXEL_LIMIT, in_file, count]
