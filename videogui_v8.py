...
class DruidGUI(object):
    ...
    def __init__(self, title, path):
        ...
        self.close_detector_button = tk.Button(self.bottom_frame, text="close analysis",
                                               command=self.close_all)
        self.close_detector_button.pack(fill=tk.X, padx=5, pady=5, side=tk.LEFT)
    ...
    def start_feed(self):
        ...
        self.v_frame.after(20, self.start_feed)
    ...
    def start_feed_b(self):
        if not self.DRUID_ANALYTICS_LOCKED and self.submit_video():
            ...
            message = 'instantiating hungarian algorithm object...'
            self.s.configure(text=message)
            m = Munkres()
...
DruidGUI('Druid Project', video_record)
